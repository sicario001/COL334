...
int main (int argc, char *argv[])
{
    bool useV6 = false;
    int mode;
    std::string modeStr, applicationDataRate = "2Mbps", channelDataRate = "6Mbps";
    if (argc!=3){
        std::cout<<"Enter the mode and corresponding data rate as argument <0 channel data rate> or <1 application data rate>";
        return 0;
    }
    else{
        modeStr = argv[1];
        ...
        if (mode==0){
            channelDataRate = argv[2];     
        }
        else{
            applicationDataRate = argv[2];
        }

        StringValue protocol = StringValue ("ns3::TcpNewReno");
    ...
    app->Setup (ns3TcpSocket, sinkAddress, 3000, -1, DataRate (applicationDataRate));
    ...
    generatePlotCwnd("CongestionWindow.cwnd", (mode==1) ? ("applicationDataRate-"+applicationDataRate) : ("channelDataRate-"+channelDataRate));
