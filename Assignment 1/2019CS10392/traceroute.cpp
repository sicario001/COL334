...
class Ping{
    ...
    static PingResult parsePingResult(string result){
        PING_STATUS status = REQUEST_TIMED_OUT;
        string ip = "";
        vector<int> rtt;
        ...
        while (std::getline(f, line)) {
            ...
            if (len>=11 && line.substr(0, 11)=="Reply from "){
                for (int i=11; i<len; i++){
                    if (line[i]==':'){
                        ...
                        if (line.substr(i+1, len-i-1)==" TTL expired in transit."){
                            status = max(status, TTL_EXPIRED_IN_TRANSIT);
                            rtt.push_back(-1);
                        }
                        else{
                ...
                status = max(status, REQUEST_TIMED_OUT);
                rtt.push_back(-1);
            }
        }
...
class TraceRoute{
    ...
    static vector<TraceRouteHopResult> runTraceRoute(string domain, int max_hops = 30){
        ...
        for (int hop = 1; hop<=max_hops; hop++){
            ...
            if (pingResult.status == REQUEST_TIMED_OUT){
                ...
                PingResult pingResult_hop = Ping::parsePingResult(Ping::PingCmd(255, pingResult.ip));
    ...
    static void print(TraceRouteHopResult traceRouteHopResult){
        ...
        for (int j=1; j<4; j++){
            ...
            else if (traceRouteHopResult.rtt[j-1]==0){
                cols[j] = "<1 ms";
            }
            else {
...
void plotXYL(string outFile, string inFile, int x, int y,string xlabel, string ylabel, string title){
    x++,y++;
    stringstream cmdstr;
    cmdstr<<"plot '"<<inFile<<"' using "<<y<<":"<<x<<" title '"<<title<<"' with linespoints linestyle 1 linecolor rgb \'#0060ad\' linetype 1 linewidth 2 pointtype 7 pointsize 1.25, ";
    Gnuplot g1;
    ...
    g1 << "MAX=GPVAL_Y_MAX\n";
    g1 << "MIN=GPVAL_Y_MIN\n";
    g1 << "set yrange [0:MAX+5]\n";
    g1 << "MAX=GPVAL_X_MAX\n";
    g1 << "MIN=GPVAL_X_MIN\n";
    g1 << "set xrange [0:MAX+5]\n";
    g1 << "set terminal png enhanced size 1920,1080\n";
    stringstream ofile;
    ofile<<"set output '"<<outFile<<"'\n";
    g1 << ofile.str();
...
void plot(vector<TraceRouteHopResult> TraceRouteResult){
    ...
    saveFile(x, y);
    plotXYL("img.png", "data.dat", 1, 0, "Hop number", "Average Round Trip Time for 3 packets (in ms)", "Plot of RTT vs hop number");
    remove("data.dat");
